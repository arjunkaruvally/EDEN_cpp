...
void LinearSM::initialize(torch::DeviceType device, int _N_f, int _N_h, float _alpha_c, float _T_d, float _T_f,
...
void LinearSM::set_interactions(torch::Tensor _Xi, torch::Tensor _Phi) {
    this->W.Xi = _Xi;
    this->W.Phi = _Phi;
}

void LinearSM::set_sequence_length(int seq_length) {
    this->N_h = seq_length;
...
States LinearSM::get_state() {
    return this->X;
}

void LinearSM::set_V_f(const torch::Tensor signal){
    auto opts = torch::TensorOptions().dtype(torch::kFloat);
    this->X.V_f = at::zeros({this->N_f, 1}, opts) + signal;
}

InteractionMatrices LinearSM::get_interactions() {
    return this->W;
}

torch::Tensor LinearSM::f_Vf(const torch::Tensor& _x) {
...
//    std::cout<<"===========================END DEBUG"<<std::endl;
    ...
    return (1/this->T_f) * (torch::matmul(this->W.Xi,this->X.V_h) - _x);
//    return (1/this->T_f) * _x;
}

torch::Tensor LinearSM::f_Vd(const torch::Tensor& _x) {
    return (1/this->T_d)*(this->X.V_f - _x);
}

torch::Tensor LinearSM::f_Xi(const torch::Tensor& _target_Vf, const torch::Tensor& _target_Vh) {
...
torch::Tensor LinearSM::f_Phi(const torch::Tensor& _target_Vf, const torch::Tensor& _target_Vh) {
    ...
    x -= this->alpha_c * torch::linalg::multi_dot({ torch::transpose(this->W.Xi, 0, 1),
                                            this->X.V_d, this->X.V_h, });
    return (1/this->Tl_phi)*x;
}

void LinearSM::update(const torch::Tensor& input_signal) {
...
void LinearSM::learn(const torch::Tensor& target_signal) {
...
torch::Tensor LinearSM::simulate_system(const torch::Tensor& input_signal,
...
bool LinearSM::evaluate_LT_learning(const int sequence_length, const int diagnostic_timescale, const int swindow,
...
std::vector<int> LinearSM::evaluate_learning(const int sequence_length, const int diagnostic_timescale, const int swindow,
...
int LinearSM::evaluate_capacity(const int max_seq_length, const int diagnostic_timescale, const int swindow,
                             const bool VERBOSE) {
...
int LinearSM::evaluate_capacity_ST(const int max_seq_length, const int diagnostic_timescale, const int swindow,
...
float LinearSM::get_energy(States& state) {
    auto energy = torch::matmul(torch::transpose(state.V_f, 0, 1), state.V_f);
//    energy += torch::matmul(torch::transpose(state.V_h, 0, 1), state.V_h);
