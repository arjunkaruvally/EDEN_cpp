...
//    if (VERBOSE) {
//        std::cout << "Xi[0]: ", model.get_interactions().Xi.index({torch::indexing::Slice(0,
//                                                                               torch::indexing::None),
//                                                        torch::indexing::Slice(0,1)});
//    }
//    input_signal.index_put_({0}, model.get_interactions().Xi.index({torch::indexing::Slice(0,
//                                                                                           torch::indexing::None),
//                                                                    torch::indexing::Slice(0,1)}));
//    // initialize all input signals from 1-end to zero
//    input_signal.index_put_({torch::indexing::Slice(1, torch::indexing::None,
//                                                    torch::indexing::None)}, 0);
...
//            std::cout<<final_output[final_output.size() - 1]<<" ";
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    program.add_argument("--max_seq_length")
            ...
            .default_value(50000).scan<'i', int>();
    ...
    program.add_argument("--pattern_bias")
            .help("timescale of the feature signal.")
    ...
    program.add_argument("--n_samples")
            .help("The number of samples to use to compute the single transition probability.")
    ...
    program.add_argument("--no_phi")
            .help("dont use the phi matrix")
            .default_value(false)
            .implicit_value(true);
    ...
    const double pattern_bias = program.get<double>("--pattern_bias");
    ...
    const int n_samples = program.get<int>("--n_samples");
    ...
    cout<<"pattern_bias: "<<pattern_bias<<std::endl;
    ...
    auto no_phi = program.get<bool>("--no_phi");
    ...
    model.pattern_bias = pattern_bias;  // bias the pattern if required
    model.NO_PHI = no_phi;
    std::cout<<"Model initialized "<<std::endl;

    int seq_capacity = model.evaluate_capacity_ST(max_seq_length, diagnostic_timescale, swindow, n_samples, VERBOSE);
    ...
    if (result_file.is_open()){
        ...
        result_file << "\"pattern_bias\": "<<pattern_bias<<"\n";
        result_file << "} \n";
        result_file.close();
    } else {
