...
struct Diagnostic {
    void operator()(float energy){
        this->energy_history.push_back(energy);
    }
...
int main(int argc, char *argv[]) {
    ...
    program.add_argument("--seed")
            ...
            .default_value(17386).scan<'i', int>();
    ...
    program.add_argument("--Tl_phi")
            ...
            .default_value(15.0).scan<'g', double>();
    ...
    program.add_argument("--data_path")
            .default_value(std::string{"data"})   // might otherwise be type const char* leading to an error when trying program.get<std::string>
    ...
    auto data_path = program.get<std::string>("--data_path");
    ...
    fs::path data_path_absolute = experiment_dir / data_dir;
    ...
    if(! model.evaluate_LT_learning(max_seq_length, diagnostic_timescale,
                                    swindow, steps_per_memory, epochs, VERBOSE, energy_trace, 1,
                                    data_path_absolute)){
